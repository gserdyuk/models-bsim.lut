...
{
  //  fprintf(stdout,"  &&&  MODEL_SPICE::precalc() \n"); 
  MODEL_CARD::precalc();
  ...
  if (info.DEVsetup) {
    ...
    ///fprintf(stdout,"  &&&  MODEL_SPICE::precalc() DEVsetup in \n"); 
   int ok = info.DEVsetup(NULL, &_gen_model_raw, ckt(), NULL);
    ///fprintf(stdout,"  &&&  MODEL_SPICE::precalc() DEVsetup out\n"); 

    assert(ok == OK);
  }else{untested();
  }
    // fprintf(stdout,"  &&&  MODEL_SPICE::precalc() exited \n");  
...
{
 //   fprintf(stdout,"void DEV_SPICE::expand()\n");
  assert_instance();
  ...
  { //-------- fix up external nodes
    ...
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point 1\n"); 

  { //------- attach model, set up matrix pointers
    _model = dynamic_cast<const MODEL_SPICE*>(find_model(_modelname));
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point AA _model                           = %d \n", _model); 
    ...
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point AB (before precalc) _model                           = %d \n", _model); 
      mm->precalc();
    // fprintf(stdout,"@@@void DEV_SPICE::expand() mm->precalc() finished \n");
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point AC (after precalc) _model                           = %d \n", _model);
      ...
      _model_spice->GENinstances = &_geninst;  // instance assigned
    // fprintf(stdout,"@@@void  point AC1 _model_spice->GENinstances, &_geninst   = %d %d\n", _model_spice->GENinstances, &_geninst);
    //fprintf(stdout,"@@@void  point AC2 _inst.lut_inited, &(_inst.lut_inited)   = %d %d\n", _inst.lut_inited, &(_inst.lut_inited));
    
      assert_model_localized();
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point AD (after chain) _model                           = %d \n", _model); 
    ...
    // fprintf(stdout,"@@@void DEV_SPICE::expand() point AD (after chain 2) _model                           = %d \n", _model); 
      //-------------
        ///fprintf(stdout,"@@@void DEV_SPICE::expand() entering DEVsetup \n");  
      int ok = info.DEVsetup(matrix, gen_model_copy, ckt(), &_num_states);
        ///fprintf(stdout,"@@@void DEV_SPICE::expand() exited DEVsetup \n"); 
      // memory pointer setup, and sets _num_states
      // undesired side effects: sets values, messes up model
      //-------------
      assert(ok == OK);
      //  fprintf(stdout,"@@@  _mode_spice->GENinstances           = %d \n", _model_spice->GENinstances);
      //  fprintf(stdout,"@@@  _model                              = %d \n", _model);
      //  fprintf(stdout,"@@@  &_model                              = %d \n", &_model);
        
      _model_spice->GENinstances = NULL;
      /*
        fprintf(stdout,"@@@void DEV_SPICE::expand() _mode_spice->GENinstances           = %d \n", _model_spice->GENinstances);
        fprintf(stdout,"@@@void DEV_SPICE::expand() _model                              = %d \n", _model);
        fprintf(stdout,"@@@void DEV_SPICE::expand() _model->_gen_model_raw.GENinstances = %d \n", _model->_gen_model_raw.GENinstances);

        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() \n");
        */
      assert_model_unlocalized();      
    /*
    assert(_model->_gen_model_raw.GENinstances == NULL);
        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() 1\n");
    assert(_model_spice);			
        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() 2\n");
    assert(_model_spice->GENmodType == 0);	
        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() 3\n");
    assert(_model_spice->GENnextModel == NULL);	
        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() 4\n");
    assert(_model_spice->GENinstances == NULL);	
        fprintf(stdout,"@@@void DEV_SPICE::expand() entering assert_model_unlocalized() 5\n");
    assert(_model_spice->GENmodName);		
        fprintf(stdout,"@@@void DEV_SPICE::expand() exiting assert_model_unlocalized() \n");
    // original
    assert_model_unlocalized();      
    */
    }
  }
    /* fprintf(stdout,"@@@void DEV_SPICE::expand() point 2\n");*/ 
  ...
  //      fprintf(stdout,"void DEV_SPICE::expand() point 3\n");
 ...
 //       fprintf(stdout,"void DEV_SPICE::expand() point 4\n");
  ...
  assert_model_unlocalized();
  assert_instance();
    // fprintf(stdout,"void DEV_SPICE::expand() - exited\n");

}
/*--------------------------------------------------------------------------*/
void DEV_SPICE::precalc()
{
    // fprintf(stdout,"  ** void DEV_SPICE::precalc() - entering\n");
  assert(_model);
  assert_instance();
  assert(info.DEVsetup);
  ...
  
  {
    ...
    MODEL model_spice_copy(*reinterpret_cast<MODEL*>(_model_spice));
    GENmodel* gen_model_copy = reinterpret_cast<GENmodel*>(&model_spice_copy);
     ///fprintf(stdout,"  **  DEV_SPICE::precalc(), before DEVsetup\n");    
    int ok = info.DEVsetup(matrix, gen_model_copy, ckt(), &num_states_garbage);
     ///fprintf(stdout,"  **  DEV_SPICE::precalc(), after DEVsetup\n");
    assert(ok == OK);
...
{
//  fprintf(stdout,"DEV_SPICE::do_tr()\n");
  ...
  //fprintf(stdout," ***** DEVSPICE::do_tr, DEVload calling, dev = %s\n", dev_type().c_str());
  //GS:----
  ...
  /* GS print 
  fprintf(stdout," ***** do_tr values\n");
  for (int ii = 0; ii < matrix_nodes()+OFFSET; ++ii) {
    fprintf(stdout, "_i0[%d], _i1[%d] = %e %e \n",ii,ii, _i0[ii], _i1[ii]);
  }
  fprintf(stdout," ***** do_tr matrices\n");
  for (int ii = 0; ii < matrix_nodes()+OFFSET; ++ii) {
    for (int jj = 0; !needs_load && jj < matrix_nodes()+OFFSET; ++jj) {
        fprintf(stdout, "_matrix[%d][%d].real(), _matrix[%d][%d].imag() = %e %e \n", ii,jj,ii,jj,_matrix[ii][jj].real(), _matrix[ii][jj].imag());		   
    }
