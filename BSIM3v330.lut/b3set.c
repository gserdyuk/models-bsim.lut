...
static char rcsid[] = "$Id: b3set.c, v3.3.0 07/29/2005 18:00:00 Xuemei Xi Release $";
...
/* Default value Processing for BSIM3 MOSFET Models */
	...
	{   if (model->BSIM3type == NMOS)
        ...
        if (!model->BSIM3kfGiven)
            model->BSIM3kf = 0.0;
#ifdef LUT
        if (!model->BSIM3vgsminGiven)
            model->BSIM3vgsmin = -5.;
        if (!model->BSIM3vgsmaxGiven)
            model->BSIM3vgsmax = +5.;
        if (!model->BSIM3vgsnumGiven)
            model->BSIM3vgsnum = 20;
        if (!model->BSIM3vdsminGiven)
            model->BSIM3vdsmin = -5.;
        if (!model->BSIM3vdsmaxGiven)
            model->BSIM3vdsmax = +5.;
        if (!model->BSIM3vdsnumGiven)
            model->BSIM3vdsnum = 20;
        if (!model->BSIM3vbsminGiven)
            model->BSIM3vbsmin = -5.;
        if (!model->BSIM3vbsmaxGiven)
            model->BSIM3vbsmax = +5.;
        if (!model->BSIM3vbsnumGiven)
	...
	{//HERE??   /* allocate a chunk of the state vector */
