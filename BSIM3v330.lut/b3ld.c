 ...
 * Modified by Gennadiy Serdyuk (for LUT) 2009-10-24
 **********/

static char rcsid[] = "$Id: b3ld.c, v3.3.0 07/29/2005 18:00:00 Xuemei Xi Release $";
...
static int cnt=0;
int NCORNERS=8;
/* prepare all 8 corners of a cube  2^3 = 8 */
int    ixa[NCORNERS], iya[NCORNERS], iza[NCORNERS];
double val[VarNo], vr[VarNo];
int  jx,jy,jz;
int check_accuracy_pass;
int lut_check_accuracy=0;
int NMAX;
double vbs_orig,vgs_orig, vds_orig;
double Ids_orig, Gds_orig, Gm_orig, Gmb_orig, Gbb_orig, Gbg_orig, Gbd_orig, Isub_orig;
double BSIM3gbs_orig, BSIM3cbs_orig, BSIM3gbd_orig, BSIM3cbd_orig, BSIM3von_orig, BSIM3mode_orig;
double diff0, diff1, diff2, diff3, diff4, diff5, diff6, diff7;
double rd0, rd1, rd2, rd3, rd4, rd5, rd6, rd7;
double mineps=1.e-18;

/* GS:
        MODEAC          - AC simulation 
        MODETRAN        - transient
        MODEINITSMSIG   - ??
        MODETRANOP      - ? may be caclulation of OP before transient?
        MODEUIC         - ? may be Initial Conditions needed ?
...
count ++;
if (inlogfile != NULL) {
    print_ckt       (inlogfile, count, ckt,   "input ckt");
    print_BSIM3model(inlogfile, count, model, "input model");
    }
#endif
...
{    for (here = model->BSIM3instances; here != NULL; 
...
#ifdef LUT
        /* initializations */
        if (!here->lut_inited && here->BSIM3lut_mode>=2){
            LUT_Init(here->lutptr,VarNo,    /* vbs, vgs, vds,*/
                model->BSIM3vbsmin,model->BSIM3vbsmax,model->BSIM3vbsnum+1,
                model->BSIM3vgsmin,model->BSIM3vgsmax,model->BSIM3vgsnum+1,
                model->BSIM3vdsmin,model->BSIM3vdsmax,model->BSIM3vdsnum+1);
            here->lut_inited=TRUE;
            }
...
#ifdef LUT
      /* 
         GS: here are defined important inputs: vbd, vgd, vgb - WRONG
         GS: WRONG! important inputs are: vbs, vgs, vds 
      Different approaches can be used. 
      first choise - square grid or triangle grid
      triangle grid may be interesting, but rectangular regular grid is simpler
      
      Triangle grid:
        A) ] if value can be computed from tables 
        B) is such - compute from tables and "go to end" 
        C) if not  - have choices:
            1) add new point - mangle vbd, vbs, vds, compute new point (triangle interpolation) and interpolate needed
            2) just compute new point from original vbd, vbs, vds, add to space.
       
       A-2 seems interesting choice
       in this case algorithm at step A) may be complex - it will require search in space to define:
        a) if point belongs to covered space
        b) if tolerance will be within required
        
       Step C) may need space reordering (as adding the point may break plane triangle to 3 and space octaedre into 4
       
       Rectangular grid:
       Algorithmically simplest - just square grid
       I) cehck if adjacent cells taken
       II) if yes - interpolate
       III) if not - fill them and interpolate 
       */     
      
      /*
      A) define additional points, run following piece multiple times.
      
      set=(vbs, vgs, vds)_1, (vbs, vgs, vds)_2, ... up to (vbs, vgs, vds)_8
      ...
      */
      ...
      /* here we have the following situations
         ...
         lut_mode = 2 - lut model needed
         lut_mode = 3 - lut mode with accuracy check
         but:
         lut_mode = 2 and point is in boundaries - proceed as lut
         lut_mode = 2 and point is out of boundaries vmin/vmax - proceed as lut_mode=1 (exact calculation w/o charge)
      */  
      if (lut_bypass) { 
        NMAX=1; /*  just one point - exact vbs, vgs, vds */
        ix=iy=iz=0;              /* not used */
        ixa[0]=iya[0]=iza[0]=0;  /* not used */
        }
      else {  /* compute cube corners */
        ix=node.x[0];
        iy=node.x[1];
        iz=node.x[2];

        /* prepare all 8 corners of a cube  2^3 = 8 */
        ixa[0]= ix;    iya[0]=   iy;    iza[0]=iz;
        ixa[1]= ix+1;  iya[1]=   iy;    iza[1]=iz;
        ixa[2]= ix;    iya[2]=   iy+1;  iza[2]=iz;
        ixa[3]= ix+1;  iya[3]=   iy+1;  iza[3]=iz;
        ixa[4]= ix;    iya[4]=   iy;    iza[4]=iz+1;
        ixa[5]= ix+1;  iya[5]=   iy;    iza[5]=iz+1;
        ixa[6]= ix;    iya[6]=   iy+1;  iza[6]=iz+1;
        ixa[7]= ix+1;  iya[7]=   iy+1;  iza[7]=iz+1;

        if (lut_check_accuracy)
            NMAX=NCORNERS+1;            /* check accuracy - compute 8 corners and exact value */
        else 
            NMAX=NCORNERS;              /* do not check accuracy - compute only 8 corners */
        }
      ...
      for (i_corner=0; i_corner<NMAX; i_corner++){
        ...
        if ( i_corner<NCORNERS && !lut_bypass ) {   /* filling corners of the cube */
            ...
            jy=iya[i_corner];
            jz=iza[i_corner];
            vbs=LUT_AxisValue(here->lutptr->x[0], jx);   
            vgs=LUT_AxisValue(here->lutptr->x[1], jy);   
            vds=LUT_AxisValue(here->lutptr->x[2], jz);   
            }   
        else    {                                   /* i_corner==NCORNERS || lut_bypass - original values */
            vbs=vbs_orig;   
            vgs=vgs_orig;   
            vds=vds_orig;   
        }
...
/* Calculate Ids */
    ...
    /*
    ...
    - store all values in correspoondent corners
    }
    ...
    */
      ...
      if ( ! LUT_isFilled(here->lutptr,jx,jy,jz) ) { // store values 
        ...
        val[8] =here->BSIM3gbs;
        val[9] =here->BSIM3cbs;
        val[10]=here->BSIM3gbd;
        val[11]=here->BSIM3cbd;
        val[12]=here->BSIM3von;
        ...
        Gbd_orig  = Gbd;
        Isub_orig = Isub;
        BSIM3gbs_orig = here->BSIM3gbs;
        BSIM3cbs_orig = here->BSIM3cbs;
        BSIM3gbd_orig = here->BSIM3gbd;
        BSIM3cbd_orig = here->BSIM3cbd;
      ...
      }   /*end if ( ! LUT_isFilled(here->lutptr,jx,jy,jz) || lut_bypass)*/
    }   /*end for */
    ...
    if (! lut_bypass)  {
        ...
        Gm  = here->BSIM3gm   = vr[2];
        Gmb = here->BSIM3gmbs = vr[3];
                  
        Gbb = here->BSIM3gbbs = vr[4];
        Gbg = here->BSIM3gbgs = vr[5];
        Gbd = here->BSIM3gbds = vr[6];

        Isub= here->BSIM3csub = vr[7];
        ...
        here->BSIM3mode = (vds_orig>=0)?1:-1;  // this follows from line 64x normal/inverse mode and used later in cdreq computation
        
        /*restore vgs, vbs, vds - they are used at convergence check below*/
        vgs=vgs_orig;
        vbs=vbs_orig;
        vds=vds_orig;
        
        /* think to include in stored values: 
        ++ here->BSIM3gbs           - used before being calculated
        ++ here->BSIM3cbs           - used before being calculated
        ++ here->BSIM3gbd           - used before being calculated
        ++ here->BSIM3cbd           - used earlier than caclulated - ned to be stored ? used also in convergence
        
        ++here->BSIM3mode           - used once before calculated - not sure if need to be stored, integer!
        
        +* here->BSIM3thetavth      - used after calculation - need not to be stored
        ++ here->BSIM3von           - used before calculated, need to store?
        +- here->BSIM3Vgsteff
        +- here->BSIM3rds
        +- here->BSIM3Abulk
        +- here->BSIM3ueff
        ...
        */
        ...
        if (lut_check_accuracy){  /* caclulate errors */
            ...
            diff2=(vr[2]-Gm_orig);   rd2=diff2/fmax(fabs(Gm_orig), mineps);
            diff3=(vr[3]-Gmb_orig);  rd3=diff3/fmax(fabs(Gmb_orig), mineps);
            diff4=(vr[4]-Gbb_orig);  rd4=diff4/fmax(fabs(Gbb_orig), mineps);
            diff5=(vr[5]-Gbg_orig);  rd5=diff5/fmax(fabs(Gbg_orig), mineps);
            ...
            fprintf(stdout, " vr[3],Gmb_orig  %g %g\n",vr[3],Gmb_orig);
            fprintf(stdout, " vr[4],Gbb_orig  %g %g\n",vr[4],Gbb_orig);
            fprintf(stdout, " vr[5],Gbg_orig  %g %g\n",vr[5],Gbg_orig);
            fprintf(stdout, " vr[6],Gbd_orig  %g %g\n",vr[6],Gbd_orig);
            fprintf(stdout, " vr[7],Isub_orig %g %g\n",vr[7],Isub_orig);   
            fprintf(stdout, " ---\n");               
            fprintf(stdout, " diff 0-7= %g %g %g %g %g %g %g \n", diff0, diff1, diff2, diff3, diff4, diff5, diff6, diff7 );
            fprintf(stdout, "rdiff 0-7= %g %g %g %g %g %g %g \n", rd0, rd1, rd2, rd3, rd4, rd5, rd6, rd7 );
            */
        }    
    }
...
if (outlogfile != NULL) {
    print_ckt       (outlogfile, count, ckt,   "output ckt");
    print_BSIM3model(outlogfile, count, model1, "output model");
    }
